...
- **Block bindings (WP 6.5+):** sources for `core/paragraph` (and other blocks that support `content` bindings) on **single event** templates or inside Query Loop with `postId` context:
  ...
  - `jardin-events/event-roles-plain` — role term names, plain text (` · `). Constants: `Jardin_Events_Block_Bindings::SOURCE_*` in [`inc/class-events-block-bindings.php`](inc/class-events-block-bindings.php). Wire bindings in the Site Editor (single event template or Query Loop with `postId` context); see [`docs/theme-handoff/`](docs/theme-handoff/README.md) for theme notes.
